...
# Constantes de Puertos Mexicanos
PUERTOS_PRINCIPALES = {
...
def generar_tendencias_mensuales():
    ...
    meses = pd.date_range(start='2025-04-01', end='2026-03-31', freq='D')
    tendencias = []
...
def page_puertos_maritimos():
    ...
    with col_btn1:
        if st.button("🔄 Recargar Datos", help="Recarga datos de puertos", key="btn_recargar_puertos"):
            ...
            st.rerun()
    with col_btn2:
        vista = st.radio("Tipo de Vista", ["Dashboard", "Tabla Detallada", "Mapa"], horizontal=True, key="radio_vista_puertos")
    with col_btn3:
        st.caption("📡 Datos actualizados al 31 de Marzo de 2026")
    ...
    st.subheader("📊 Indicadores Clave de Desempeño Nacional", divider="blue")
    ...
    with kpi_col1:
        st.metric(
            ...
            delta="TEU",
            help="Total de contenedores (TEU) movidos mensualmente en todos los puertos"
        )
    
    with kpi_col2:
        sat_change = kpis['saturacion_promedio'] - 55  # Compare con baseline 55%
        st.metric(
            label="📈 Saturación Promedio",
            value=f"{kpis['saturacion_promedio']:.1f}%",
            delta=f"{sat_change:+.1f}%" if sat_change != 0 else None,
            help="Porcentaje promedio de ocupación en todos los puertos"
        )
    ...
    with kpi_col3:
        st.metric(
            ...
            delta="USD",
            help="Valor aproximado del movimiento comercial mensual"
        )
    ...
    with kpi_col4:
        st.metric(
            ...
            delta_color="inverse",
            help="Puertos con saturación > 80%"
        )

    # ========== SECCIÓN 2: FILTROS Y VISTAS ==========
    if vista == "Dashboard":
        ...
        with viz_col1:
            # Gráfico de barras: Throughput por Puerto
            fig_bar = px.bar(
                ...
                hover_data={'Saturación': ':.1f', 'Estado': True}
            )
        ...
        with viz_col2:
            # Gráfico de gauge: Saturación promedio
        ...
        # Gráfico de mapa de calor: Saturación
        st.markdown("### 🔥 Matriz de Saturación por Puerto")
        ...
        st.markdown("### 📈 Tendencias Mensuales de Throughput")
        ...
        st.markdown("### ⚙️ Distribución de Operaciones")
        ...
        with col_op2:
            # Scatter: Operaciones vs Saturación
        ...
        st.markdown("### ⏱️ Tiempos Promedio de Espera")
        ...
        st.markdown("### 📋 Tabla Completa de Puertos")
        ...
        st.markdown("### 🗺️ Mapa Geográfico de Puertos")
    ...
    st.markdown("---")
    st.markdown("### 🔗 Integraciones con Otras Secciones")
    ...
    with int_col1:
        st.info(
            ...
            **📦 Flujos de Carga**
            ...
            - Seguimiento de contenedores puerto→aduana
            """
        )
    ...
    with int_col2:
        st.warning(
            ...
            **🛣️ Corredores Logísticos**
            - Rutas terrestres desde puertos mexicanos
    ...
    with int_col3:
        st.success(
            """
            **👥 Monitor Laboral**
            - Disponibilidad de transportistas para carga marítima
            - Coordinación temporal con operaciones portuarias
            """
        )
    ...
    # Puertos críticos
    criticos = df_puertos[df_puertos['Saturación'] > 80]
    if not criticos.empty:
        with st.container(border=True):
            st.error(f"🔴 **CRÍTICO**: {len(criticos)} puerto(s) con saturación > 80%")
            for idx, row in criticos.iterrows():
                st.write(f"  • **{row['Puerto']}**: {row['Saturación']:.1f}% - Tiempo espera: {row['Tiempo_Promedio_Espera']}h")
    
    # Puertos en situación normal
    normales = df_puertos[df_puertos['Saturación'] <= 60]
    if not normales.empty:
        with st.container(border=True):
            st.success(f"✅ **OPERATIVO**: {len(normales)} puerto(s) en condiciones normales")
            for idx, row in normales.head(3).iterrows():
                st.write(f"  • **{row['Puerto']}**: {row['Saturación']:.1f}% - Capacidad disponible: {int((1-row['Saturación']/100)*row['Capacidad_Mensual']):,} TEU/mes")

    # ...existing code from the extracted Streamlit page...
    # (The rest of the Streamlit UI and visualizations are copied as in app.py)

    # --- Insert the rest of the extracted logic here, as per the original function ---
