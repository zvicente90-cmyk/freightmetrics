...
with metric_col1:
    st.metric("💱 USD/MXN", metrics['usd_mxn'])

with metric_col2:
    st.metric("📅 Fecha", metrics['fecha'])

with metric_col3:
    st.metric("🕐 Hora (CDMX)", metrics['hora'])
...
with cta_col2:
    if st.button("📚 Explorar Academia", use_container_width=True, key="academia_cta"):
...
<div style='text-align: center; color: #999; font-size: 0.85rem; padding: 20px 0;'>
    © 2026 FreightMetrics. Todos los derechos reservados.
    <br>
    <small>Datos obtenidos de CBP, INEGI, SCT y otras fuentes gubernamentales verificadas.</small>
</div>
""", unsafe_allow_html=True)
